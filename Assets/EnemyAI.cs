...
public class EnemyAI : MonoBehaviour
{
	...
	void FixedUpdate ()
    {
        ...
        if (transform.tag == "Carrier")
        {
            ...
            {
                transform.position = Vector3.Lerp(transform.position, new Vector3(playerPosition.x, playerPosition.y, playerPosition.z - 1000f), 0.15f * Time.deltaTime);
            }
        }
        else
        {
            ...
            if (warped) //If you are warped in.
            {
                ...
                transform.position = Vector3.Lerp(transform.position, new Vector3(playerPosition.x, playerPosition.y, playerPosition.z - 1000f), 0.25f * Time.deltaTime);

                if (transform.position.z <= -350f)   //If you go to far, shut off.
                {
